...
class MyGame(arcade.Window):
    ...
    def on_draw(self):
        ...
        for i in self.points:
            if i[3] == 0:
                if i[2] == arcade.color.WHITE:
                    arcade.draw_circle_filled(i[0], i[1], self.radius, arcade.color.WHITE)
                    continue
                ...
                continue
            if i[2] == arcade.color.WHITE:
                arcade.draw_rect_filled(arcade.rect.XYWH(i[0], i[1], self.radius * 2,
            ...
            arcade.draw_rect_filled(arcade.rect.XYWH(i[0], i[1], self.radius * 2, self.radius * 2), color)

    def on_mouse_press(self, x, y, button, modifiers):
        # Добавьте координаты (x, y) в self.points
        if button == arcade.MOUSE_BUTTON_LEFT and modifiers & arcade.key.MOD_CTRL:
            self.points.append((x, y, arcade.color.WHITE, 0))
        elif button == arcade.MOUSE_BUTTON_RIGHT and modifiers & arcade.key.MOD_CTRL:
            self.points.append((x, y, arcade.color.WHITE, 1))
        elif button == arcade.MOUSE_BUTTON_LEFT:
            self.points.append((x, y, self.counter, 0))
        elif button == arcade.MOUSE_BUTTON_RIGHT:
            self.points.append((x, y, self.counter, 1))
        self.counter += 1
